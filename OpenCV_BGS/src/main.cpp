...
int main(int argc, const char ** argv)
{
    ...
    // Create directories for the entire path
    try {
        cv::utils::fs::createDirectories(config.output_dir);
        cout << "Directory created successfully!" << std::endl;
    } catch(const std::exception& e) {
        cerr << "Failed to create directory: " << e.what() << std::endl;
    }
    ...
    cv::imwrite(config.output_dir +"/bgs_" + stringify(0) +".png", bgmask);
    // cv::imwrite(config.output_dir +"/BGModel/bgm_" + stringify(0) +".png", bgmask);

    cv::imwrite(config.output_dir +"/bgs_" + stringify(1) +".png", bgmask);
